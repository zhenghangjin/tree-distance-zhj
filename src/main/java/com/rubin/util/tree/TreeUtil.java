...
import com.rubin.util.tree.cost.TriFunction;
import com.rubin.util.tree.operate.SimpleTreeOperation;
...
public class TreeUtil {
    ...
    public static <T> int treeEditDistance(TreeNode<T> root1, TreeNode<T> root2, List<TreeOperation> ops,
        ...
        final BiMap<TreeNode<T>, Integer> order1 = postOrder(root1); // 后序遍历 root1
        final BiMap<TreeNode<T>, Integer> order2 = postOrder(root2); // 后序遍历 root2
        System.out.println("order1=" + JSON.toJSONString(order1.keySet()));
        System.out.println("order2=" + JSON.toJSONString(order2.keySet()));

        int[] l1 = leftMostDescendants(order1); // 计算每个节点最左边的后代的标记
        int[] l2 = leftMostDescendants(order2);

        int[] kr1 = keyRoots(root1, order1); // 根节点 或 非第一个兄弟节点
    ...
    private static void print(CostAndOps[][] td){
        System.out.println("===============================");
        System.out.print("\t");
        for(int kkk=0; kkk<10; kkk++){
            System.out.print("i"+kkk+"\t");
        }
        System.out.println("");
        for(int i=0; i<td.length; i++){
            System.out.print("i"+i+"\t");
            for(int j=0; j<td[i].length; j++){
                if(td[i] == null || td[i][j]==null){
                    System.out.print("\t");
                } else {
                    System.out.print(td[i][j].cost+"\t");
                }
            }
            System.out.println("");
        }
     ...
     * 最左边的后代
     */
    private static <T> int[] leftMostDescendants(BiMap<TreeNode<T>, Integer> order) {// 最左边的后代
    ...
    private static <T> void forestDistance(int kr1, int kr2, int[] l1, int[] l2, CostAndOps[][] td,
        ...
        for (int di = l1[kr1]; di <= kr1; di++) {
            final Integer cost = costFunction.apply(
                    order1.inverse().get(di),
                    order2.inverse().get(l2[kr2] - 1),
        ...
        for (int dj = l2[kr2]; dj <= kr2; dj++) {
            final Integer cost = costFunction.apply(
                    order1.inverse().get(l1[kr1] - 1),
                    order2.inverse().get(dj),
